...
import { changeGateLayout } from './circuit';
...
/**
 * Function called to generate a prompt to select gate layout
 * @category ux
 * @exports createGateLayoutPrompt
 */
export function createGateLayoutPrompt() {
    if (globalScope.settings.isDIN) {
        $("input[name='gateLayoutType'][value='DIN']").prop("checked", true);
    } else {
        $("input[name='gateLayoutType'][value='ANSI']").prop("checked", true);
    }

    $("#gateLayoutDialog").dialog({
        resizable: false,
        width: "auto",
        buttons: [
            {
                ...
                click() {
                    const isDIN =
                        $("input[name='gateLayoutType']:checked").val() ===
                        "DIN";
                    changeGateLayout(isDIN);
                    $(this).dialog("close");
                },
            },
        ],
